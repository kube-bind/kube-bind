...
func New(caBundleFile string, listener net.Listener, addrOverride string) (*Server, error) {
	...
	staticUserProvider := NewStaticUserProvider()
	staticUserProvider.Set(&mockoidc.MockUser{
	...
	server, err := mockoidc.NewServer(&mockoidc.ServerConfig{
		...
		UserProvider: staticUserProvider,
	})
	if err != nil {
...
// Config returns the OIDC configuration for clients to connect to the embedded OIDC server.
// It is partially auto-generated by mockoidc and partially overridden to match kube-bind specific URLs.
...
func ensureOfflineAccessScope() {
	...
	if slices.Contains(mockoidc.ScopesSupported, offlineAccess) {
