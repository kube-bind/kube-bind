...
// ClusterScoped wraps a generic patcher to make it look like a namespaced patcher, but
// it ignores the namespace argument (which is empty string in cluster-scoped code).
func ClusterScoped[R any](p Patcher[R]) NamespacedPatcher[R] {
	return func(string) Patcher[R] {
		return p
	}
}
...
func NewCommitter[R any, Sp any, St any](patcher NamespacedPatcher[R]) func(context.Context, *Resource[Sp, St], *Resource[Sp, St]) error {
	...
	return func(ctx context.Context, old, obj *Resource[Sp, St]) error {
		...
		_, err = patcher(obj.Namespace).Patch(ctx, obj.Name, types.MergePatchType, patchBytes, metav1.PatchOptions{}, subresources...)
		if err != nil {
			return fmt.Errorf("failed to patch %s %s/%s: %w", focusType, ns, name, err)
		}
