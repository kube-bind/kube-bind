...
func TestDownstreamCreation(t *testing.T) {
	...
	if v, ok := createdObj.GetAnnotations()["kube-bind.io/owner"]; !ok || v != "provider" {
		t.Error("created object did not have 'kube-bind.io/owner: provider' annotation", createdObj)
	}
	if v, ok := providerObj.GetAnnotations()["kube-bind.io/owner"]; !ok || v != "provider" {
...
func TestUpstreamCreation(t *testing.T) {
	...
	if v, ok := createdObj.GetAnnotations()["kube-bind.io/owner"]; !ok || v != "consumer" {
		t.Error("created object did not have 'kube-bind.io/owner: consumer' annotation", createdObj)
	}
	if v, ok := providerObj.GetAnnotations()["kube-bind.io/owner"]; !ok || v != "consumer" {
		t.Error("pre-existing object was not updated to be the owner")
	}
}

func defaultNamespace(upstreamNamespace string) (*v1alpha2.APIServiceNamespace, error) {
	return &v1alpha2.APIServiceNamespace{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "default",
			Namespace: "kube-bind",
		},
		Spec: v1alpha2.APIServiceNamespaceSpec{},
		Status: v1alpha2.APIServiceNamespaceStatus{
			Namespace: "cluster-x-default",
		},
	}, nil
}
...
func TestDownstreamUpdate(t *testing.T) {
	...
	if v, ok := updateObj.GetAnnotations()["kube-bind.io/owner"]; !ok || v != "consumer" {
		t.Error("updated object did not have 'kube-bind.io/owner: consumer' annotation")
	}
}
...
func TestUpdate(t *testing.T) {
	...
	if v, ok := updateObj.GetAnnotations()["kube-bind.io/owner"]; !ok || v != "provider" {
		t.Error("updated object did not have 'kube-bind.io/owner: provider' annotation")
	}
}
			...
			obj.SetUnstructuredContent(
				map[string]interface{}{
					"metadata": map[string]interface{}{
						"name":        "dummy",
						"namespace":   "cluster-x-default",
						"annotations": map[string]interface{}{"kube-bind.io/owner": "provider"},
					},
					"data": map[string]interface{}{
						"username": "user",
						"password": "pass",
