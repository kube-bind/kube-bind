...
type APIServiceExportRequestSpec struct {
	...
	// namespaces specifies the namespaces, we should bootstrap as part of this request.
	// When objects originate from provider side, consumer does not always know the nessesary details.
	// This field allows provider to pre-heat the necessary namespaces on provider side by creating
	// APIServiceNamespace objects attached to the APISericeExport. More namespaces can be created later by the consumer.
