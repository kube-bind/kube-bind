...
type APIServiceExportRequestClusterInterface interface {
	...
	APIServiceExportRequestClusterExpansion
}
...
func (c *aPIServiceExportRequestsClusterInterface) List(ctx context.Context, opts v1.ListOptions) (*kubebindv1alpha1.APIServiceExportRequestList, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).APIServiceExportRequests(v1.NamespaceAll).List(ctx, opts)
}

// Watch begins to watch all APIServiceExportRequests across all clusters.
func (c *aPIServiceExportRequestsClusterInterface) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.clientCache.ClusterOrDie(logicalcluster.Wildcard).APIServiceExportRequests(v1.NamespaceAll).Watch(ctx, opts)
}

// APIServiceExportRequestsNamespacer can scope to objects within a namespace, returning a APIServiceExportRequestInterface.
...
func (n *aPIServiceExportRequestsNamespacer) Namespace(namespace string) APIServiceExportRequestInterface {
