...
class AuthService {
  ...
  async checkAuthentication(): Promise<AuthCheckResult> {
    try {
      ...
      const authCheckUrl = clusterId
        ? `/ping?cluster_id=${clusterId}`
        : "/ping";
      ...
      if (error instanceof StructuredError) {
        ...
        if (
          kubeError.code === ErrorCodes.AUTHENTICATION_FAILED ||
          kubeError.code === ErrorCodes.AUTHORIZATION_FAILED
        ) {
          return {
  ...
  restorePreservedParams(): boolean {
